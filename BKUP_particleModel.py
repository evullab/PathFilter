...
class Particle(object):
    ...
    def setpath(self, noisytable, limitsteps = 26000):
...
class ParticleTable(object):
    ...
    def __init__(self, phystable, kapv = kapv_def, kapb = kapb_def, kapm = kapm_def, perr = perr_def, nparticles = 5, timeperstep = .1, newp = NEWP, tmp = TEMP):
    ...
    def getDecision(self, tcut = 10., ps = None):
        decs = map(lambda p: p.getdecision(self.t, tcut),self.particles)
        if ps is None: ps = self.getPartPs()
        
        redp = 0.
        greenp = 0.
        totp = sum(ps)
        if totp < self.newp: return 'U'
        
        for d, p in zip(decs, ps):
            if d == RED: redp += p
            if d == GREEN: greenp += p
        
        if (redp / totp) > 0.99: return 'R'
        if (greenp / totp) > 0.99: return 'G'
...
if __name__ == "__main__":
    pg.init()
    clock = pg.time.Clock()
    sc = pg.display.set_mode((1200,900),pg.RESIZABLE)
