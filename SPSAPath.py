...
def unnormalizeParams(theta):
    parts = theta[0]
    cutoff = 10^(-theta[1])
    tlim = theta[2]
    temp = 1 / theta[3]
    return [parts,cutoff,tlim,temp]

class PathSPSA ( object ):
    ...
    def __init__ ( self, loss_function, a_par = 1e-6, noise_var=0.01, trials = [], args=(), \
            min_vals=None, max_vals=None, param_tolerance=None, \
            function_tolerance=None, max_iter=200 ):
    ...
    def calc_loss ( self, theta , trials):
        """Evalute the cost/loss function with a value of theta"""
        retval = self.loss ( unnormalizeParams(theta), trials )
        return retval

    def minimise ( self, theta_0, ens_size=2, report=None, samples = 24):
        ...
        if report is not None:
            rfl = open(report,'w')
            rfl.write('Iteration,Part,DecLim,TimeLim,Temp,ak,ck')
            for i in range(samples): rfl.write(',Sample' + str(i))
            rfl.write('\n')
        n_iter = 0
        ...
        while  (np.linalg.norm(theta_saved-theta)/np.linalg.norm(theta_saved) >\
            ...
            thistrial = random.sample(self.trials, samples)
            ...
            for j in np.arange ( ens_size ):
                ...
                #``theta_minus``
                j_plus = self.calc_loss ( theta_plus, thistrial )
                j_minus = self.calc_loss ( theta_minus, thistrial )
            ...
            j_new = self.calc_loss ( theta, thistrial )
            # Be chatty to the user, tell him/her how it's going...
            realparams = unormalizeParams(theta)
            print "\tIter %05d" % n_iter, j_new, ak, ck, realparams
            if report is not None:
                rfl.write(str(n_iter) + ',' + str(realparams[0]) + ',' + str(realparams[1]) + ',')
                rfl.write(str(realparams[2]) + ',' + str(realparams[3]) + ',' + str(ak) + ',' + str(ck))
                for t in trials:
                    rfl.write(',' + t.name)
                rfl.write('\n')
