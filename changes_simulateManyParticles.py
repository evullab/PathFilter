...
def singleSim(tr, pathid, numpart, pcut, declim = 10., temp = 1., ofl = None, kapv = kapv_def, kapb = kapb_def, kapm = kapm_def, perr = perr_def, timeperstep = .1):
    ...
    if ofl:
        ...
        ofl.write(tr.name + ',' + str(pathid) + ',' + str(t) + ',' + partTab.getDecision(declim,ps) + ',1.0')
    ...
    while running:
        ...
        decs.append(dec)
        rejs.append(partTab.lastrej)
        if ofl:
            poss = partTab.getPartPos()
            ds = map(lambda p: makedec(p.getdecision(partTab.t, declim)),partTab.particles)
            ofl.write(tr.name + ',' + str(pathid) + ',' + str(t) + ',' + dec + ',' + str(partTab.lastrej))
            for pos, p, d in zip(poss, ps, ds):
                ofl.write(',' + str(pos[0]) + ',' + str(pos[1]) + ',' + str(p) + ',' + d)
            ofl.write('\n')
...
def trialSim(tr, numpart, pcut, declim = 10., temp =1., nsims = 50, path = '.', kapv = kapv_def, kapb = kapb_def, kapm = kapm_def, perr = perr_def, timeperstep = .1):
    ...
    fl.write('Trial,SimNo,Time,Decision,NewParts,')
    ...
    for i in range(nsims):
        try:
            ds, rejs = singleSim(tr,i,numpart,pcut,declim,temp,fl,kapv,kapb,kapm,perr,timeperstep)
        except:
            print "Error in", trnm
            raise
        nsw = 1
    ...
    totrej = map(lambda x: x / float(nsims), sumrej)

    print "Done:",trnm
    return [trnm,reds,greens, switch, switms, sumrej]
...
def simulateThemAll(numpart, pcut, declim, temp=1., nsims = 50, kapv = kapv_def, kapb = kapb_def, kapm = kapm_def, perr = perr_def, timeperstep = .1):
    ...
    folnm = os.path.join('.','ParticleOutput',"CountRej_OccPaths_Part{:02d}_Prob{:1.1e}_TLim{:02d}_Temp{:0.2f}".format(numpart,pcut,int(declim),temp))
    ...
    # Summarize output
    sumfl = open(os.path.join(folnm,'CountRej_Summary_Part{:02d}_Prob{:1.1e}_TLim{:02d}_Temp{:0.2f}.csv'.format(numpart,pcut,int(declim),temp)),'w')
    sumfl.write('Trial,T,Red,Green,Switches,Rejuvinations')
    for i in range(1,9):
    ...
    for o in outs:
        ...
        for i in range(len(reds)):
            tm += .1
            sumfl.write(tnm+','+str(tm)+','+str(reds[i])+','+str(greens[i])+','+str(switch[i]) +',' + str(rejs[i]))
            for k in range(0,8):
